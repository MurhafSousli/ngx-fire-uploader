...
export class FileUploaderComponent implements OnInit, OnDestroy {
  ...
  constructor(private _manager: FireUploaderManager, private _storage: AngularFireStorage) {
    ...
    if (!this._state.active && this._state.files.length) {
      from(this._state.files).pipe(
        ...
        finalize(() => {
          ...
          const uploaded = this._state.files.filter(file => file.state === 'success');
          this.completeEmitter.emit(uploaded);

          // Emits the URLs of the uploaded files.
          const downloadURLs = uploaded.map(file => file.state.downloadURL);
  ...
  removeFile(file: FileItem) {
    if (file.state.state === 'success') {
      file.delete();
    } else if (file.state.state === 'running') {
      file.cancel();
    } else {
      this.removeEmitter.emit(file);
    }
  ...
  cancel(remove = true) {
    ...
    this._state.files.map((file: FileItem) => {
      if (remove && file.state.state === 'success') {
        file.delete();
      } else {
        file.cancel();
      }
  ...
  /**
   * Loop over all file items states then reduces them to root state
   */
